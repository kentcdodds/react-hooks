...
- We can use `React.useEffect` hook to make other backend actions to happen
  everytime a render update is done.
- Lazy `useState()` initialization lets us pass a function, instead of a value
  to `useState`. This function gets called only when needed.
- `React.useEffect` allows a dependency array argument, which lets this
  `useEffect` to take effect only when an element in the dependency array
  changes.
- We can create custom hooks, by naming a function to begin with `use`.
- The extra credit-4 lecture is quite insightful. Kent shows how custom hooks
  can be made modular in many ways.
