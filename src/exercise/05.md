...
- If we want to use libraries in react that need direct access to DOM elements,
  we will need to ask react to return the DOM element after rendering and use it
  by using `useEffect` hook.
- [When the ref attribute is used on an HTML element, the ref created in the constructor with React.createRef() receives the underlying DOM element as its current property.](<https://legacy.reactjs.org/docs/refs-and-the-dom.html#:~:text=When%20the%20ref%20attribute%20is%20used%20on%20an%20HTML%20element%2C%20the%20ref%20created%20in%20the%20constructor%20with%20React.createRef()%20receives%20the%20underlying%20DOM%20element%20as%20its%20current%20property.>)
