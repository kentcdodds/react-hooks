...
- Implemented tic-tac-toe by treating each square to be bound by a `useState`
  hook. Not sure if this is the best way, looking forward to the lecture.
- We can implement by using a single `useState` for the whole array of 9
  elements - not fully sure which is better in what way.
